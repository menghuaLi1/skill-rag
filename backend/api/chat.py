...
async def chat(payload: ChatRequest):
    ...
    async def event_generator():
        ...
        try:
            stream = agent_manager.astream(payload.message, history, session_id=payload.session_id)
            stream_iterator = stream.__aiter__()
            started_at = time.monotonic()

            while True:
                remaining = CHAT_STREAM_TIMEOUT_SECONDS - (time.monotonic() - started_at)
                if remaining <= 0:
                ...
                try:
                    event = await asyncio.wait_for(stream_iterator.__anext__(), timeout=remaining)
                ...
                if event_type == "token":
                    ...
                    persist_segments()
                    await agent_manager.maintain_hierarchical_memory(payload.session_id)
            ...
            message = str(exc) or "聊天流超时，系统已自动停止。"
            persist_segments(fallback_content=message)

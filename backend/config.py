...
class Settings:
    ...
    mineru_command_template: str = "mineru -p {input} -o {output}"
...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        mineru_enabled=_parse_bool(_first_config_value("MINERU_ENABLED"), default=False),
        mineru_command_template=_first_config_value("MINERU_COMMAND_TEMPLATE")
        or "mineru -p {input} -o {output}",
        mineru_timeout_seconds=_parse_positive_int(
            _first_config_value("MINERU_TIMEOUT_SECONDS"),
            default=300,
        ),
    )

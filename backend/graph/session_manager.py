...
class SessionManager:
    ...
    def _read_session_file(self, session_id: str) -> dict[str, Any]:
        ...
        raw.setdefault("short_term_summary", "")
    ...
    def load_session_for_agent(self, session_id: str) -> list[dict[str, str]]:
        ...
        short_term_summary = record.get("short_term_summary", "").strip()
        if short_term_summary:
            merged.append(
                {
                    "role": "assistant",
                    "content": f"[以下是近期对话的滑窗摘要]\n{short_term_summary}",
                }
            )
    ...
    def compact_short_term_window(
        ...
        *,
        ...
        record = self._read_session_file(session_id)
        messages = record.get("messages", [])
        if keep_last_messages < 1 or len(messages) <= keep_last_messages:
            return {"archived_count": 0, "remaining_count": len(messages)}

        archived = messages[:-keep_last_messages]
        remaining = messages[-keep_last_messages:]

        archive_path = self.archive_dir / f"{session_id}_window_{int(time.time())}.json"
        archive_payload = {
            "session_id": session_id,
            "archived_at": time.time(),
        ...
        existing_summary = record.get("short_term_summary", "").strip()
        if existing_summary:
            record["short_term_summary"] = f"{existing_summary}\n---\n{summary.strip()}"
        else:
            record["short_term_summary"] = summary.strip()
        record["messages"] = remaining
        self._write_session(record)
        return {
            "archived_count": len(archived),

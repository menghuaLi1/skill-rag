...
class MemoryIndexer:
    ...
    def _maintain_memory_fact_file(self) -> None:
        if not self._memory_path.exists():
            return
        ...
        kv_pattern = re.compile(r"^-\s*([^:：]{1,120})[:：]\s*(.+)$")
        ...
        for raw_line in lines:
            ...
            expire_match = expires_pattern.search(line)
            if expire_match:
                try:
                    expires_at = time.mktime(time.strptime(expire_match.group(1), "%Y-%m-%d"))
                except ValueError:
                    expires_at = now_ts + 1
        ...
        if conflicts:
            conflict_path = self._storage_dir / "fact_conflicts.json"
        ...
        if not rewritten:
            rewritten = "# Long-term Memory\n"
        rewritten += "\n"
        self._memory_path.write_text(rewritten, encoding="utf-8")
    ...
    def rebuild_index(self) -> None:
        ...
        self._maintain_memory_fact_file()

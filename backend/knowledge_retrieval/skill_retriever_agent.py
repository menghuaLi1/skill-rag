...
MAX_SKILL_TOOL_CALLS = 18
MAX_SKILL_SAME_TOOL_STREAK = 5
MAX_SKILL_UPDATE_EVENTS = 500
MAX_SKILL_RUNTIME_SECONDS = 90
...
class SkillRetrieverAgent:
    ...
    async def astream(self, query: str) -> AsyncIterator[dict[str, Any]]:
        ...
        async for mode, payload in agent.astream(
            ...
            if time.monotonic() - started_at > MAX_SKILL_RUNTIME_SECONDS:
                stop_reason = "Skill 检索超时，已自动停止以避免死循环。"
                break
            ...
            if update_event_count > MAX_SKILL_UPDATE_EVENTS:
                stop_reason = "Skill 检索更新事件过多，已自动停止。"
                break
                ...
                for agent_message in update.get("messages", []):
                    ...
                    if tool_calls:
                        for tool_call in tool_calls:
                            ...
                            if tool_call_count > MAX_SKILL_TOOL_CALLS:
                                stop_reason = "Skill 检索工具调用次数超过上限，已自动停止。"
                                break
                            if same_tool_streak > MAX_SKILL_SAME_TOOL_STREAK:
                                stop_reason = "Skill 检索重复工具调用过多，已自动停止。"
                                break
        ...
        result = self._build_result(final_content, recorded_tools)
        if stop_reason:
            result.status = "uncertain"
            result.reason = stop_reason
        yield {"type": "skill_result", "result": result.to_dict()}
